...
const emptyCat: CatDraft = { name_uz: '', name_ru: '', slug: '', parent_id: '', gram_price: '' };
  ...
  return (
    <div className="space-y-2 rounded-lg border border-border p-3">
      <div className="grid grid-cols-2 gap-2">
        ...
        <NumberInput label="Gramm narxi" size="sm" value={draft.gram_price} onChange={(v) => setDraft({ ...draft, gram_price: v })} min={0} step={10_000} suffix="so'm" thousands />
      </div>
...
function CategoryTab() {
  ...
  const toBody = (d: CatDraft) => ({
    ...
    gram_price: d.gram_price === '' ? null : d.gram_price,
  });
  ...
  const startEdit = (c: CategoryOut) => {
    ...
    setEditDraft({
      ...
      gram_price: c.gram_price != null ? Number(c.gram_price) : '',
    });
  };
            ...
            <div key={c.id} className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2">
              <div className="min-w-0">
                ...
                <p className="text-2xs text-muted">
                  {c.slug}
                  {c.gram_price != null && ` · ${formatMoney(Number(c.gram_price))}/g`}
                </p>
              </div>
              <div className="flex items-center gap-2">
